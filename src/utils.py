...
def compute_errors(args, depth, pred, mask):
    pred = nn.functional.interpolate(pred, depth.shape[-2:], mode='bilinear', align_corners=True)
    # scale, shift = compute_scale_and_shift(pred.clone(), depth.clone(), mask.clone())
    # pred = scale.view(-1, 1, 1, 1) * pred + shift.view(-1, 1, 1, 1)
